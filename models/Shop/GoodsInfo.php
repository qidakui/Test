...
class GoodsInfo extends \CActiveRecord
{
	...
	{
		return array(
				'goods_spec' => array(self::BELONGS_TO,get_class(GoodsSpec::model()), 'goods_spec_id', 'join_with_in' => true),
				'goods_attr' => array(self::BELONGS_TO,get_class(GoodsAttr::model()), 'goods_attr_id', 'join_with_in' => true),
		);
	}
	...
	protected function afterSave(){
		$column_name = 'GoodsInfo';
		if($this->isNewRecord){
			$column_name = '新增GoodsInfo';
			$operate = 'add';
		}else{
			if($this->_delete == 1){
				$column_name = '删除GoodsInfo';
				$operate = 'del';
			}else{
				$column_name = '编辑GoodsInfo';
				$operate = 'edit';
			}
		}
		OperationLog::addLog(OperationLog::$operationGoodsInfo, $operate, $column_name, $this->id, $this->old_date, $this->attributes);
		$this->old_date = '';
	}
	...
	public function createGoodsInfo($info){
		$model = new self();
		foreach($info as $k=>$v){
			$model->$k = $v;
		}
	...
	public function updateGoodsInfo($info){
