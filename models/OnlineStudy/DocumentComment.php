...
use OperationLog;
class DocumentComment extends \CActiveRecord
{
    public $status_name;
    ...
    public function del_comment($id){
        ...
        if(!empty($comment)){
            ...
            DocumentComment::model()->updateCounters(array('reply_count'=>(-1-$sub_comment_count)),':id=id',array(':id'=>$comment->document_id));
            OperationLog::addLog(OperationLog::$operationVideoComment, 'del', '删除评论', $id, array(), array());
            \CreditLog::addCreditLog(\CreditLog::$creditDocument,\CreditLog::$typeKey[11],$id,'subtract','',$comment->user_id);
            return true;
        }
    }
