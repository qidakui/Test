...
class GcmcCase extends \CActiveRecord
{
    public $status_name;
    ...
    public function getlist($con, $orderBy, $order, $limit, $offset){
        ...
        if(!empty($con)){
            foreach($con as $key => $val){
                if($key=='starttime'){
                    ...
                    $criteria->addCondition('title like \'%'.$con['title'].'%\' OR author like \'%'.$con['title'].'%\' ');
                }elseif( !in_array($key, array('title','starttime','endtime')) ){
        ...
        $status_arr = [0=>'已删除',1=>'通过',2=>'待审核',3=>'未通过'];
        foreach($ret as $k => $v){
            $v = $v->attributes;
            ...
            if($is_score){
                ...
                $_v['classify'] = '';
                $_v['type'] = '案例';
                $statistical = GcmcScore::model()->statistical(3,$v['id']);
                $v = $_v + $statistical;
            }
			$data[] = $v;
        }
